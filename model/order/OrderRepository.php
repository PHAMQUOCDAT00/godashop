...
class OrderRepository{
	protected function fetchAll($condition = null, $sort = null)
	{
		...
		if ($result->num_rows > 0) 
		{
			...
			{
				$order = new Order($row["id"], $row["created_date"], $row["order_status_id"], $row["staff_id"], $row["customer_id"], $row["shipping_fullname"], $row["shipping_mobile"], $row["payment_method"], $row["shipping_ward_id"], $row["shipping_housenumber_street"], $row["shipping_fee"], $row["delivered_date"],$row["payment"]);
				$orders[] = $order;
			}
		}
