<table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?=$order->getShippingHousenumberStreet()?>, <?=$ward->getName()?>, <?=$district->getName()?>,
                <?=$province->getName()?></td>
            <td><?=!empty($staff) ? $staff->getName() : ""?></td>
            <td>
                <?php if ($order->getStatusId() == 1): ?>
                <a class="btn btn-primary btn-sm" href="index.php?c=order&a=confirm&order_id=<?=$order->getId()?>"
                    onclick="return confirm('Bạn muốn xác nhận đơn hàng này phải không?')">Xác nhận</a>
                <?php endif?>
            </td>
            <td> <input type="button" onclick="alert('hỗ trợ sau')" value="Sửa" class="btn btn-warning btn-sm"></td>
            <td>
                <a class="btn btn-danger btn-sm" href="index.php?c=order&a=delete&order_id=<?=$order->getId()?>"
                    onclick="return confirm('Bạn muốn xóa đơn hàng này phải không?')">Xóa</a>
            </td>
        </tr>
        <?php endforeach ?>
    </tbody>
